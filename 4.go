...
`
    ...
    byrRegex := regexp.MustCompile("byr:([0-9]{4}) ")
    iyrRegex := regexp.MustCompile("iyr:([0-9]{4}) ")
    eyrRegex := regexp.MustCompile("eyr:([0-9]{4}) ")
    hgtRegex := regexp.MustCompile("hgt:([0-9]+)(cm|in) ")
    hclRegex := regexp.MustCompile("hcl:#[0-9a-f]{6} ")
    eclRegex := regexp.MustCompile("ecl:(amb|blu|brn|gry|grn|hzl|oth) ")
    pidRegex := regexp.MustCompile("pid:[0-9]{9} ")

    p2ValidPassports := 0

    for _, passport := range passports {
        passport = strings.ReplaceAll(passport, "\n", " ") + " "
        byrMatches := byrRegex.FindStringSubmatch(passport)
        iyrMatches := iyrRegex.FindStringSubmatch(passport)
        eyrMatches := eyrRegex.FindStringSubmatch(passport)
        hgtMatches := hgtRegex.FindStringSubmatch(passport)
        hclValid   := hclRegex.MatchString(passport)
        eclValid   := eclRegex.MatchString(passport)
        pidValid   := pidRegex.MatchString(passport)
        ...
        if (len(byrMatches) == 2 &&
            ...
            byr, _ := strconv.Atoi(byrMatches[1])
            iyr, _ := strconv.Atoi(iyrMatches[1])
            eyr, _ := strconv.Atoi(eyrMatches[1])
            hgt, _ := strconv.Atoi(hgtMatches[1])

            if (byr >= 1920 && byr <= 2002 &&
                iyr >= 2010 && iyr <= 2020 &&
                eyr >= 2020 && eyr <= 2030 &&
                (hgtMatches[2] == "cm" && hgt >= 150 && hgt <= 193 ||
                 hgtMatches[2] == "in" && hgt >= 59  && hgt <= 76)) {
