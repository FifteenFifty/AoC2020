...
"flnhl gkddl jhqpdf jkvcbf xqcb gcbbnqb ddmg ppqs bnl jrpx cdslv bxlc tfgf msrnhf strd vxzpfp jsdgn kcgtlb cljvdxp glxxp pvx prrfm qqmjlr drgdhp tmp jrj zrvtg rmxzzf zhpv vnxgpb ldggb ksrh rzlkxt qqbfj lnmv njtfs vjbgmpr pdpgm tpbtpd vxpbn dzqs dnbhhv dpkkzdgs dqz cbhfpzp djldkx mqlnbjq gkfzqvc lfmng pqnx jvjhx gmtkh jbcjk dvzqx pgnk szdxjlb ttkn dqrvvk cltpv (contains wheat, nuts)",
  ...
  p1, allergenMap := P1(input)
  p2 := P2(allergenMap)

  fmt.Printf("P1: %d, P2: %s\n", p1, p2)
}

func P1 (input []string) (int, map[string][]string) {
  ret := 0

  wordMatch := regexp.MustCompile("([a-z]+)")
  ...
  for _, line := range input {
    ...
    ingredients := wordMatch.FindAllString(parts[0], -1)
    allergens   := wordMatch.FindAllString(parts[1], -1)

    for _, allergen := range allergens {
      knownIngrdients, ok := allergenMap[allergen]
      if !ok {
        allergenMap[allergen] = ingredients
      } else {
        allergenMap[allergen] = Intersection(knownIngrdients, ingredients)
      }
    }
  ...
  for _, values := range allergenMap {
    for _, allergenIngredient := range values {
      ingredientMap[allergenIngredient] = 0
    }
  }

  for _, noAllergens := range ingredientMap {
    ret += noAllergens
  }

  return ret, allergenMap
}

func P2 (input map[string][]string) string {
  uniqueAllergens := []string {}
  keys            := []string {}
  collectKeys     := true

  // Uniquify the allergen lists
  for {
    changed := false
    for k, v := range input {
      if len(v) == 1 {
        // Inefficient but it works well enough
        uniqueAllergens = append(uniqueAllergens, v[0])
      } else {
        input[k] = Difference(v, uniqueAllergens)
        changed = true
      }
  ...
  for _, key := range keys {
    uniqueAllergens = append(uniqueAllergens, input[key][0])
  }

  return strings.Join(uniqueAllergens, ",")
}
  ...
  for _, aElement := range a {
    for _, bElement := range b {
      if aElement == bElement {
        intersection = append(intersection, aElement)
      }
    }
  }
  return intersection
...
func Difference(include []string, exclude []string) []string {
  differenceMap := make(map[string]bool)
  difference    := []string {}

  for _, aElement := range include {
    differenceMap[aElement] = true
  }
  for _, bElement := range exclude {
    differenceMap[bElement] = false
  }

  for k, v := range differenceMap {
    if v == true {
